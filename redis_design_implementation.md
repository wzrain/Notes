...
# Chapter 19 事务
事务将多个命令打包，然后一次性按顺序执行，事务执行期间服务器不会中断事务去执行其他客户端的命令请求。事务首先以一个MULTI命令开始，然后将多个命令放到事务中，最后由EXEC命令将事务提交给服务器执行。

## 19.1 事务的实现
MULTI命令将客户端从非事务状态切换至事务状态，通过flags属性打开REDIS_MULTI标识。之后如果客户端发送EXEC、DISCARD、WATCH、MULTI命令中的一个，服务器立即执行，否则将命令放入事务队列，给客户端返回QUEUED回复。\
每个客户端都有自己的事务状态，保存于客户端状态的mstate属性中：
```C++
typedef struct redisClient {
    // ...
    multiState mstate;
...
```
事务队列以先进先出的方式保存入队命令。执行EXEC命令时，服务器遍历该队列，执行队列中保存的命令，并移除REDIS_MULTI标识，清零入队命令计数器并释放事务队列，将执行结果返回给客户端。

## 19.2 WATCH命令的实现
WATCH命令是一个乐观锁，在EXEC命令执行之前监视任意数量的数据库键，在EXEC命令执行时检查被监视的键是否至少有一个已经被修改，如果是的话服务器拒绝执行事务，向客户端返回事务执行失败的空回复。\
每个Redis数据库都保存一个watched_keys字典，字典的键是被某个WATCH命令监视的数据库键，值为链表，每个链表项为监视对应数据库键的客户端。
```C++
typedef struct redisDb {
    // ...
    dict *watched_keys;
...
```
执行WATCH命令，客户端可以在watched_keys字典中与被监视的键进行关联。\
所有对数据库进行修改的命令，比如SET、LPUSH、SADD、ZREM、DEL、FLUSHDB等待，执行后会调用multi.c/touchWatchKey函数对watched_keys字典检查，查看是否有客户端正在监视被修改过数据库键，如果有的话将对应客户端的REDIS_DIRTY_CAS标识打开，表明事务安全性被破坏。\
当客户端执行EXEC命令时服务器查看该标识是否打开，如果打开则说明至少一个键被修改，则服务器拒绝执行客户端提交的事务。

## 19.3 事务的ACID性质
### 19.3.1 原子性
Redis的事务是原子性的 (?)，事务队列的命令要么全部执行要么全部不执行。如果事务中某个命令无法被执行（“编译”错误，例如参数个数错误），则所有命令都不会被执行。\
Redis的事务和传统关系型数据库事务的区别在于Redis不支持回滚，如果事务中的某个命令执行期间出现错误（“运行时”错误，例如错误对字符串键执行列表键的命令），则后续命令也会继续执行下去，前面的命令也不会受影响。\
Redis事务的运行时错误通常是由编程错误产生的，通常只出现在开发环境中，而很少在生产环境中出现，因此没必要为Redis开发复杂的事务回滚功能。

### 19.3.2 一致性
一致性指数据符合数据库本身的定义和要求，没有包含非法或者无效的错误数据。Redis通过谨慎的错误检测和简单的设计来保证事务的一致性。\
如果事务入队命令时发现命令不存在或者格式不正确（“编译”错误），则Redis拒绝执行。因此一致性不会被错误事务影响。Redis 2.6.5之前这些错误命令会被跳过。\
命令实际执行错误时（“运行时”错误）不会影响事务的其他命令，出错的命令会被服务器识别并进行相应的错误处理，不会对数据库做任何修改，因此不会对一致性产生影响。\
服务器停机时，如果服务器运行在无持久化模式下，重启后数据库为空，数据总是一致的。如果运行在RDB或者AOF模式下，中途停机可以通过持久化文件恢复到一致状态。

### 19.3.3 隔离性
隔离性是指多个事务并发执行不互相影响，与串行执行事务结果相同。Redis使用单线程方式执行事务以及事务中的命令，并且执行事务期间不会对事务进行中断，因此事务总是串行方式运行，具有隔离性。
...
# Chapter 24 监视器
通过执行MONITOR命令，客户端将自己变成一个监视器，实时接收并打印服务器当前处理的命令请求相关信息。每当一个客户端向服务器发送一条命令请求时，服务器除了会处理这条命令请求之外，还会关于这条命令请求的信息发送给所有监视器。

## 24.1 成为监视器
MONITOR命令打开客户端的REDIS_MONITOR标识，并将客户端添加到服务器状态redisServer结构的monitors链表末尾。

## 24.2 向监视器发送命令信息
服务器每次处理命令请求前调用replicationFeedMonitors函数，将命令请求发送给各个监视器。该函数构造一个包含命令信息的消息，遍历监视器链表，将消息发送给每个监视器。