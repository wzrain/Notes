...
# Chapter 5 链路层：链路、接入网和局域网
## 5.4 交换局域网
交换机运行在链路层，不识别网络层地址，不使用路由选择算法确定交换机的路径。

### 5.4.1 链路层寻址和ARP
主机和路由器的网络接口（适配器）具有链路层地址（MAC地址），链路层交换机没有与接口关联的链路层地址，因为交换机的任务是透明地在主机和路由器之间承载数据报，主机和路由器不必明确将链路层帧寻址到交换机。对大多数局域网MAC地址为6字节，即共有2^48个可能的MAC地址。\
没有两块适配器有相同的MAC地址，MAC地址具有扁平结构（而不是层次结构），不论适配器到哪里MAC地址都不会变化。IP地址具有层次结构（网络部分和主机部分），主机移动时IP地址会改变。\
一个适配器向另一个适配器发送一个帧时，发送方将接收方的MAC地址插入该帧中，并将该帧发送的局域网上。发送方也可以将帧广播到它的所有接口，因此一个适配器可以接收一个并非向它寻址的帧，只有接收到MAC地址与自己的地址匹配的帧时，适配器才会提取封装的数据报向网络层传递。可以使用特殊的MAC广播地址（48个1组成的地址）让所有的适配器处理对应的帧。\
MAC地址独立于网络层地址的原因是局域网不只是用于IP和因特网，如果适配器也被分配IP地址则不能很好地支持其他协议。另外如果适配器使用网络层地址则此地址必须存储在适配器的RAM中，每次适配器移动或者重启时需要重新配置。

地址解析协议（ARP）用于转换IP地址和MAC地址。发送主机通过ARP确认目的主机的MAC地址。ARP只为在同一个子网的主机和路由器接口解析IP地址（DNS为因特网中任何地方的主机解析主机名）。\
每个主机或者路由器在内存中有一个ARP表，包含IP地址到MAC地址的映射关系。ARP表同时包含TTL值，表示从表中删除对应映射的时间。如果发送主机的ARP表中还没有关于目的主机IP地址的映射，发送方会向它的适配器传递一个ARP查询分组，使用MAC广播地址发送。子网中每个适配器都会接收到这一查询分组，与被查询的IP地址相匹配的ARP模块会将自己的MAC地址返回，以便于发送方更新ARP表。之后发送方可以发送IP数据报，封装在链路层帧中，该帧的目的MAC就是之前响应ARP查询请求的主机或路由器。ARP协议是即插即用的，ARP表自动建立，不需要系统管理员来配置。\
ARP分组封装在链路层帧中，包含网络层地址，因此体系结构上讲ARP位于链路层之上，但是ARP分组包含链路层地址，因此也可认为是链路层协议。

对于一个有两个接口的路由器，它有两个IP地址，两个ARP模块和两个适配器（对应两个MAC地址）。当一个发送方将数据报发送给子网外的接收方时，对应的链路层帧的目的MAC地址应该是对应的第一跳路由器接口的MAC地址。路由器通过转发将数据报传递给对应的接口（适配器），适配器将数据报封装到新的帧中，发送给新的子网，这个新的帧的目的MAC地址是最终目的主机的MAC地址或者下一个路由器的输入接口的MAC地址。这些MAC地址都可以通过对应ARP模块获得。

### 5.4.3 链路层交换机
交换机的任务是接收链路层帧并将其转发至出链路，这个过程对主机和路由器透明。交换机输出接口设有缓存，以应对帧到达输出接口的速率超过该接口的链路容量。

过滤决定一个帧应当被转发还是丢弃，转发决定一个帧被导向哪个接口并将其移动至对应的接口。过滤和转发基于交换机表完成。交换机表的一个表项包括一个MAC地址、通向该MAC地址的交换机接口以及表项放置在表中的时间。\
如果一个目的MAC地址为a的帧从交换机的接口x到达，如果表中没有a对应的表项，则交换机向除了x以外的所有接口转发该帧的副本。如果表中a对应的接口为x，则无需进行任何转发，丢弃该帧执行过滤功能即可（因为这意味着该帧已经在包含目的主机的局域网被广播过了）。如果a对应的接口不是x，则转发到对应接口。\
交换机表是自学习得到的，初始为空，对于每个接收到的帧，交换机存储源MAC地址以及对应的接口以及当前时间。如果一段时间后交换机没有收到以该地址作为源地址的帧，则删除该地址（这样如果有适配器离开，原来的MAC地址会被清除掉）。\
交换机是即插即用设备，不需要网络管理员或者用户的干预。

交换机可以消除碰撞，因为它可以缓存帧并且不会同时传输多余一个帧，其最大聚合带宽为所有接口速率之和，因此其提供了相比于广播链路高的多的性能改善。此外交换机将链路彼此隔离，不同的链路可以在不同速率下在不同媒体上运行。交换机便于网络管理，可以检测异常适配器，收集带宽的统计数据、碰撞率、流量类型等。

交换机的优点是即插即用，具有较高的分组过滤和转发速率。不过为了防止广播帧的循环，交换机的活跃拓扑限制为树，另外一个大型交换网络会有很大的ARP表，生成可观的ARP流量和处理量，且交换机对广播风暴无保护措施（如果主机传输大量广播帧，交换机将转发所有这些帧，导致以太网崩溃）。\
因为网络寻址为层次寻址，因此分组通常不会通过路由器循环，同时路由器对广播风暴提供了保护。路由器的缺点是不即插即用，需要人为配置IP地址。\
对于几百台主机的小网络，有几个局域网段，使用交换机即可，对于几千台主机的大型网络，路由器提供了更健壮的流量隔离和广播风暴的控制。
